...
public class SheinRealClient extends AbstractPlatformClient implements SheinClient {
    ...
    public List<UnifiedOrder> fetchRecentOrders(Long shopId) {
        // 真实 Shein API 待对接：打 warn 日志，返回空列表降级，避免抛异常中断订单同步流程。
        log.warn("真实 Shein API 待对接，使用 mock 数据降级：fetchRecentOrders 返回空列表 shopId={} appKey={}",
                shopId, mask(appKey));
    ...
    public boolean markShipped(String platformOrderNo, String trackingNo) {
        // 真实 Shein API 待对接：打 warn 日志，返回 false 降级，避免抛异常；
        // 上游可据此决定是否重试，而非强制中断。
        log.warn("真实 Shein API 待对接，使用 mock 数据降级：markShipped 返回 false orderNo={} trackingNo={}",
