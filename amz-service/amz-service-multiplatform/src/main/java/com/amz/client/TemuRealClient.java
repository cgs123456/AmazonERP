 ...
 * 待实现项：
 * <ul>
 *   <li>appKey + appSecret 生成 sign</li>
 *   <li>/order/list 拉取订单</li>
 *   <li>/order/detail 订单详情</li>
 *   <li>/logistics/ship 上传发货信息</li>
 * </ul>
...
public class TemuRealClient extends AbstractPlatformClient implements TemuClient {
    ...
    public List<UnifiedOrder> fetchRecentOrders(Long shopId) {
        // 真实 Temu API 待对接：打 warn 日志，返回空列表降级，避免抛异常中断订单同步流程。
        log.warn("真实 Temu API 待对接，使用 mock 数据降级：fetchRecentOrders 返回空列表 shopId={} appKey={}",
                shopId, mask(appKey));
    ...
    public boolean markShipped(String platformOrderNo, String trackingNo) {
        // 真实 Temu API 待对接：打 warn 日志，返回 false 降级，避免抛异常；
        // 上游可据此决定是否重试，而非强制中断。
        log.warn("真实 Temu API 待对接，使用 mock 数据降级：markShipped 返回 false orderNo={} trackingNo={}",
