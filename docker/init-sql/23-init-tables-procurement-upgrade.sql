...
-- 数据库: amz_procurement
-- 升级内容：供应商档案/采购计划/采购明细/入库批次/FBA货件/头程费用
-- ============================================
...
CREATE TABLE IF NOT EXISTS amz_supplier (
    ...
    payment_terms VARCHAR(50) COMMENT '付款方式（如款到发货/月结30天）',
...
CREATE TABLE IF NOT EXISTS amz_supplier_product (
    ...
    is_preferred TINYINT DEFAULT 0 COMMENT '是否首选供应商(1=是)',
...
CREATE TABLE IF NOT EXISTS amz_purchase_plan (
    ...
    asin VARCHAR(20),
    suggested_qty INT NOT NULL COMMENT '建议采购数量(来自补货引擎)',
...
-- ============================================
-- 5. 采购订单表增加字段（审批流 + 供应商关联）
-- ============================================
ALTER TABLE amz_purchase_order
    ADD COLUMN IF NOT EXISTS supplier_id BIGINT DEFAULT NULL COMMENT '供应商 ID' AFTER supplier_name,
    ADD COLUMN IF NOT EXISTS plan_id BIGINT DEFAULT NULL COMMENT '关联采购计划 ID' AFTER supplier_id,
    ADD COLUMN IF NOT EXISTS approver VARCHAR(50) DEFAULT NULL COMMENT '审批人' AFTER status,
    ADD COLUMN IF NOT EXISTS approved_time DATETIME DEFAULT NULL COMMENT '审批时间' AFTER approver,
    ADD COLUMN IF NOT EXISTS total_quantity INT DEFAULT NULL COMMENT '总数量' AFTER total_amount,
    ADD COLUMN IF NOT EXISTS currency VARCHAR(10) DEFAULT 'CNY' COMMENT '币种' AFTER total_amount;

-- 更新状态枚举说明（不需要修改列，仅文档更新）：
-- DRAFT → PENDING_APPROVAL → APPROVED → ORDERED → PARTIAL_RECEIVED → RECEIVED → CLOSED
-- 异常：QC_FAILED → RETURNED；CANCELED
...
CREATE TABLE IF NOT EXISTS amz_inventory_batch (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    shop_id BIGINT NOT NULL,
    batch_no VARCHAR(32) NOT NULL UNIQUE COMMENT '批次号(年月日+序号)',
    ...
    available_quantity INT NOT NULL COMMENT '可用数量(扣减已出库)',
    unit_cost DECIMAL(10,2) NOT NULL COMMENT '批次单位成本(含采购+运费+关税分摊)',
    ...
    status VARCHAR(20) DEFAULT 'ACTIVE' COMMENT 'ACTIVE(可用)/EXPIRED(过期)/DEPLETED(耗尽)',
    create_time DATETIME DEFAULT CURRENT_TIMESTAMP,
    update_time DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    INDEX idx_shop_sku (shop_id, sku),
...
CREATE TABLE IF NOT EXISTS amz_fba_shipment (
    ...
    warehouse_id BIGINT COMMENT '源仓库 ID(本地仓/海外仓)',
    ...
    status VARCHAR(20) DEFAULT 'CREATED' COMMENT 'CREATED/PLANNING/READY_TO_SHIP/SHIPPED/IN_TRANSIT/DELIVERED/CHECKED_IN/RECEIVING/CLOSED/EXCEPTION',
...
-- 初始示例数据
-- ============================================
INSERT IGNORE INTO amz_supplier (shop_id, supplier_name, supplier_code, contact_name, contact_phone, rating, on_time_delivery_rate, quality_pass_rate, price_competitiveness, response_speed, total_orders, total_amount, status)
VALUES
...
INSERT IGNORE INTO amz_supplier_product (supplier_id, shop_id, sku, asin, supplier_offer_id, supply_price, moq, lead_time_days, is_preferred, status)
VALUES
